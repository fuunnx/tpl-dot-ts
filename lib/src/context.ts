...
export async function runWithContexts<R>(contexts: ProvidedContext[], fn: () => Promise<R>): Promise<R> {
...
export function createContext<T>(name: string, getDefaultValue: () => T = () => fail(`No default value for context: ${name}`)): Context<T> {
