...
function createDefineService(params: DockerParams) {
   ...
   */
  return function defineService<Name extends string, S extends NxDefinitionsService>(name: Name, conf: S | Falsy):Record<string, never> | {[K in Name]: S} {
    ...
    const finalName = `${params.prefix}-${params.target}-${name}`
