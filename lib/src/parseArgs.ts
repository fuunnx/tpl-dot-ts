...
type ParsedArgs<IArgs extends Args> = {
  [K in keyof IArgs]: IArgs[K] extends readonly string[] ? IArgs[K][number] : IArgs[K] extends boolean ? boolean : IArgs[K]
}
...
export function parseArgs<IArgs extends Args>(args: IArgs): ParsedArgs<IArgs> {
  for (const [key, value] of Object.entries(args)) {
    ...
    if(isBool) {
      ...
      const option = new Option(`--${key} <true|false>`).argParser(parseBool).default(value)
    ...
    if(isChoices) {
      ...
      const option = new Option(`--${key} <${key}>`).choices(value).default(defaultValue)
      program.addOption(option)
      continue
    }
