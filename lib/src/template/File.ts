...
export class TemplateFile<T = unknown> implements ITemplateFile<T> {
	...
	withContext = (...contexts: ProvidedContext[]) => {
		return new TemplateFile(this.#hoistedContent, [
			...this.contexts,
			...contexts,
		])
	}
	...
	content = () => {
		return runWithContexts(
			this.contexts,
			async () => await this.#hoistedContent(),
		)
	}
	...
	write = async (outputFileName: string, relativeTo?: string) => {
		return writeFile(
			await this.materialize(outputFileName),
			relativeTo ? path.join(relativeTo, outputFileName) : outputFileName,
		)
	}
}
